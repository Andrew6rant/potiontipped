...
public class BothTippedMiningToolItem extends MiningToolItem implements TippedTool{
    private final MiningToolItem parent;

    public BothTippedMiningToolItem(MiningToolItem parent) {
        ...
        if(group.equals(PotionTipped.TIPPED_GROUP)){
            Registry.POTION.forEach((potion -> {
                if(!potion.getEffects().isEmpty() && !potion.hasInstantEffect()){
                    TippedItemUtil.expandStacks(this, potion, stacks, TippedItemUtil.TippedType.BOTH);
                }
            }));
        }
    }
